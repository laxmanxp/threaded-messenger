...
export function LocalDemoApp({ onSwitchToOrg }: { onSwitchToOrg?: () => void }) {
      ...
      <div className="flex items-center gap-2 border-b border-[var(--border)] bg-[var(--bg-header)] px-3 py-1.5 pt-[max(0.35rem,env(safe-area-inset-top))]">
        ...
        {onSwitchToOrg ? (
          <button
            ...
            className="min-h-11 rounded-md px-2 text-[13px] text-[var(--accent)] hover:bg-[var(--bg-hover)]"
          >
            Org mode
          </button>
        ) : null}
